...
def estimate_approach_queue_resolution_time(
    ...
    # Расчёт по полосам
    for iLanes in range(InLanesCount):  # Пробегаем по входным полосам
        for iOuts in range(nActualOutCount):  # Пробегаем по выходам
            rowContribution = 0  # Вклад текущей полосы и выхода
            Km = 1.0  # Коэффициент радиуса поворота

            # Получаем радиус поворота, если доступен
            if iLanes < len(turningRadiuses) and iOuts < len(turningRadiuses[iLanes]):
                specificRadius = turningRadiuses[iLanes][iOuts]
                if specificRadius > 0:
            ...
            for k in range(len(reductionByDynamicCoeffs)):  # Пробегаем по типам ТС
                rowContribution += LaneQueue[iLanes][k] * reductionByDynamicCoeffs[k] * Km

            # Применение коэффициента условий Ki (если доступно)
            if iLanes < len(reductionByConditionCoeffs):
                rowContribution *= reductionByConditionCoeffs[iLanes]
